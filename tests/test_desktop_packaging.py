...
def test_tauri_bundle_declares_self_contained_sidecar() -> None:
    config = json.loads(
        (_REPOSITORY_ROOT / "apps" / "desktop" / "src-tauri" / "tauri.conf.json").read_text()
    )
    assert config["bundle"]["externalBin"] == ["binaries/osca-sidecar"]
    assert "npm run build:sidecar" in config["build"]["beforeBuildCommand"]


def test_sidecar_build_is_pinned_and_target_specific() -> None:
    ...
    assert "rustc" in source
    assert 'f"{_BINARY_NAME}-{target}"' in source
    assert '"--onefile"' in source
    assert '"--collect-submodules"' in source
    assert '"osca"' in source


def test_native_broker_prefers_bundled_sidecar_outside_development() -> None:
    source = (
        _REPOSITORY_ROOT / "apps" / "desktop" / "src-tauri" / "src" / "lib.rs"
    ).read_text()
    assert "bundled_sidecar_path()" in source
    assert 'join("osca-sidecar")' in source
