...
def _evidence_chain(root: Path) -> dict[WorkspaceSection, str]:
    ...
    _write(acquisition, {"family": "osca.historical-acquisition.evidence", "version": "2.0.0", "status": "succeeded", "symbol": "XBTUSD", "timeframe": "1d", "dataset_revision_id": "revision-1", "acquisition_id": "acquisition-1", "completed_at": "2026-08-02T14:05:59Z", "redistribution_enabled": False})
    _write(experiment, {"family": "osca.ml-experiment.result", "version": "1.0.0", "status": "succeeded", "experiment_id": "experiment-1", "dataset_revision_id": "revision-1", "symbol": "XBTUSD", "timeframe": "1d", "completed_at": "2026-08-02T14:10:00Z"})
    _write(diagnostic, {"family": "osca.prediction-diagnostic.result", "version": "1.0.0", "status": "review_required", "diagnostic_id": "diagnostic-1", "experiment_id": "experiment-1", "completed_at": "2026-08-02T14:11:00Z"})
    _write(manifest, {"family": "osca.research-pipeline.manifest", "version": "1.0.0", "status": "diagnostic_not_eligible", "run_id": "run-1", "experiment_id": "experiment-1", "completed_at": "2026-08-02T14:12:00Z"})
    return {
        WorkspaceSection.ACQUISITIONS: "acquisitions:historical-acquisition/kraken.json",
        WorkspaceSection.EXPERIMENTS: "experiments:research-evidence/run-1/experiment.json",
        WorkspaceSection.DIAGNOSTICS: "diagnostics:research-evidence/run-1/diagnostic.json",
        WorkspaceSection.PIPELINE_RUNS: "pipeline_runs:research-evidence/run-1/manifest.json",
    }
...
def test_research_evidence_uses_dedicated_sections(tmp_path: Path) -> None:
    ...
    assert sections[WorkspaceSection.DIAGNOSTICS].items[0].status is WorkspaceItemStatus.REVIEW_REQUIRED
    assert sections[WorkspaceSection.PIPELINE_RUNS].items[0].status is WorkspaceItemStatus.NOT_ELIGIBLE
    assert all("/research-evidence/" not in str(item.artifact_uri) and "/historical-acquisition/" not in str(item.artifact_uri) for item in sections[WorkspaceSection.REPORTS].items)
...
def test_detail_lineage_filters_and_portable_export(tmp_path: Path) -> None:
    ...
    filtered = service.filtered_snapshot(tmp_path, WorkspaceFilter(section=WorkspaceSection.EXPERIMENTS, symbol="XBTUSD", timeframe="1d"))
...
def test_api_cli_and_export_use_equivalent_contracts(tmp_path: Path, capsys: object) -> None:
    item_ids = _evidence_chain(tmp_path)
    experiment_id = item_ids[WorkspaceSection.EXPERIMENTS]
    client = TestClient(create_app(storage_root=tmp_path))
    api_snapshot = client.get("/api/evidence", params={"section": "experiments", "symbol": "XBTUSD", "timeframe": "1d"})
    assert api_snapshot.status_code == 200
    assert api_snapshot.json()["total_items"] == 1
    exit_code = main(["--storage-root", str(tmp_path), "--snapshot", "--section", "experiments", "--symbol", "XBTUSD", "--timeframe", "1d"])
    assert exit_code == 0
...
def test_corrupt_research_evidence_is_not_healthy(tmp_path: Path) -> None:
    ...
    snapshot = AnalystWorkspaceService().snapshot(tmp_path)
    diagnostics = next(section for section in snapshot.sections if section.section is WorkspaceSection.DIAGNOSTICS)
