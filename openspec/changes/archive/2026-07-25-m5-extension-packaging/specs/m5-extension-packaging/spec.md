...
Extension installation records and activation decisions SHALL be persisted in governed SQLite metadata storage with stable identifiers and queryable history.

#### Scenario: Activation decision stored
- **WHEN** activation is decided for a stored installation
- **THEN** the decision is persisted and can be queried by installation or package

### Requirement: CLI administration

The CLI SHALL expose metadata-only installation, activation decision, and installation-listing operations backed by the lifecycle store.

#### Scenario: Extension listed
- **WHEN** a local owner lists extensions after installation
- **THEN** the persisted installation record is returned without executing extension code
