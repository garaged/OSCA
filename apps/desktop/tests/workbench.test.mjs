...
test("D5 frontend uses only typed desktop workbench methods for authoritative analysis", async () => {
  ...
  const appApi = await readFile(new URL("../src/api.ts", import.meta.url), "utf8");
  assert.match(appApi, /sample\.import/);
  assert.match(appApi, /comparison_imports/);
  assert.match(appApi, /SampleImportedDataset/);
