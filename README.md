...
The M0-M12 architecture and lifecycle roadmap is complete. P1-P5 established provider governance and operator visibility. P6-P8 provide a usable no-cost local evidence path: import user-supplied OHLCV, generate deterministic research observations, and run a transparent backtest into a linked local paper-evaluation record. P9 is complete with deterministic SEC fixture replay and explicit opt-in SEC EDGAR preview; FRED remains policy-blocked and optional.

P10 is the current implementation candidate. It adds capability-based runtime routing for governed local OHLCV and SEC enrichment, explicit stale/unavailable/blocked evidence, and partial batch behavior so a macro policy block does not stop non-macro work. P11 remains the next practical milestone for an approachable analyst workspace. Production ingestion, paid provider promotion, recommendations, broker execution, autonomous trading, and real-capital orders remain disabled.
...
32. [P10 milestone](docs/milestones/p10/README.md)
33. [P10 user testing quickstart](docs/milestones/p10/user-testing-quickstart.md)
34. [P9-P10 requirements and traceability reconciliation](docs/governance/p9-p10-reconciliation.md)
35. [Remaining P milestone roadmap](docs/milestones/remaining-p-roadmap.md)
...
The P10 routing workflow is documented in [P10 user testing quickstart](docs/milestones/p10/user-testing-quickstart.md). It inspects the capability matrix, selects local OHLCV and SEC fixture evidence, confirms FRED's policy block, and demonstrates that mixed non-macro/macro work reports `partial` while preserving successful non-macro decisions.

## Capability boundary

- **OHLCV:** governed local Parquet payloads in P10.
...
- **Product independence:** the absence of a macro provider does not block research, backtesting, SEC enrichment, routing, or the planned analyst workspace.
